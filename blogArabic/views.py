...
def index(request):
           ...
           article_read = articles.objects.filter().order_by('-read')[:4]
           ...
           return render(request, 'home/index.html', {'full_name': request.user.username, 'first_article':first, 
            'fourth_article':fourth, 'all_articles': all_articles, 'five_articles': article_read, 'solo': solo, 
            'solo1': solo1, 'featured': featured}) 
...
def show_article(request, id, slug):
    ...
    three = articles.objects.order_by('-id')[:4]# when we have 10 post with this less 2 article[::-2]
